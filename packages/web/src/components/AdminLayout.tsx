...
export default function AdminLayout() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (role !== 'admin') { supabase.auth.signOut(); navigate('/admin'); return; }
      setAdmin({ email: data.session.user.email || 'admin' });
    });
    ...
    <div className="min-h-screen bg-gray-950 flex">
      <aside className="w-56 bg-gray-900 border-r border-gray-800 min-h-screen flex flex-col">
        ...
        <div className="p-3 border-t border-gray-800 flex items-center justify-between">
          ...
          <button onClick={() => supabase.auth.signOut()} className="text-xs text-gray-500 hover:text-red-400">Salir</button>
        </div>
      </aside>
