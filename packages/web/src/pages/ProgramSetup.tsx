...
export default function ProgramSetup() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    await supabase.from('programs').insert({ business_id: businessId, title: form.title, target: Number(form.target), reward: form.reward });
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-6">
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <button type="submit" disabled={saving} className="bg-primary-600 text-white px-6 py-2 rounded-lg text-sm font-semibold hover:bg-primary-700 transition-colors disabled:opacity-50">{saving ? 'Creando...' : 'Crear programa'}</button>
          </form>
        </div>
      )}
